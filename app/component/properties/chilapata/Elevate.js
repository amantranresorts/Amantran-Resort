  ...
  return (
    <div className="container mx-auto px-4 sm:px-10 lg:px-40 py-6">
      ...
      <div className="text-left mb-10">
        ...
        <h2 className="text-3xl md:text-4xl font-raleway mt-2">The Best Resort In Chilapata - Every Room Ensures Comfort</h2>
        <p className="text-black font-open-sans mt-2 md:pr-20">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {rooms.map((room) => (
          <div key={room.id} className="relative group rounded-lg overflow-hidden shadow-lg aspect-video">
            <Image 
              src={room.image} 
              alt={room.title} 
              width={857} 
            ...
            <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition duration-300 bg-black/50 flex flex-col justify-center items-center text-center p-4 z-10">
              ...
              <div className="flex flex-wrap justify-center gap-2">
                <Link href="/book" className="px-3 py-1 bg-white text-black rounded hover:bg-amber-800 hover:text-white transition duration-300 text-sm">
                  Book Now
                </Link>
                <button 
        ...
        <div className="fixed inset-0 z-50 flex justify-center items-center bg-black bg-opacity-50">
          <div className="bg-white p-6 rounded-lg shadow-xl w-11/12 md:w-3/4 lg:w-1/2 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl md:text-2xl font-bold">{activeRoom.title}</h2>
              <button 
                onClick={closeModal} 
            ...
            <div className="flex justify-end space-x-3">
              <Link href="/book" className="bg-amber-800 text-white px-4 py-2 rounded hover:bg-amber-700 transition-colors">
                Book Now
              </Link>
              <button onClick={closeModal} className="bg-gray-200 text-gray-800 px-4 py-2 rounded hover:bg-gray-300 transition-colors">
                Close
              </button>
            </div>
          </div>

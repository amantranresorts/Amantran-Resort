...
const Navbar = () => {
  ...
  const currentProperty = propertyList.find(property => property.href === pathname)
  ...
  const underlineClass = "absolute left-0 -bottom-1 w-full h-0.5 bg-amber-600 transform scale-x-0 group-hover:scale-x-100 transition-transform duration-300 ease-in-out origin-left"
  ...
  const renderNavLink = (href, label, onClickHandler = null) => {
    ...
    return (
      <li onClick={onClickHandler} className={navItemContainerClass}>
        <Link href={href}>
          <span className={`relative ${activeTextClass} py-1 hover:text-amber-700 transition-colors duration-300`}>
            {label}
    ...
    <li className={navItemContainerClass}>
      ...
      >
        <div className="flex items-center cursor-default">
          ...
          <AiOutlineDown className={`ml-1 transition-transform duration-300 ${desktopPropertiesOpen ? 'rotate-180' : ''}`} />
          <span className={underlineClass}></span>
        </div>
        <div className={`absolute left-0 top-full mt-2 bg-white shadow rounded min-w-[430px] transition-all duration-200 ease-in-out z-20 transform ${desktopPropertiesOpen ? 'opacity-100 pointer-events-auto translate-y-0' : 'opacity-0 pointer-events-none translate-y-2'}`}>
            ...
            {propertyList.map((property) => (
              <Link key={property.href} href={property.href}>
                <div className="flex items-center space-x-2 px-2 py-2 hover:bg-gray-100 hover:text-amber-800 rounded transition-transform duration-200 hover:scale-105 w-full">
                  ...
                  <span className="whitespace-normal text-[12px]">{property.name}</span>
                </div>
              </Link>
            ))}
          </div>
  ...
  const renderMobileProperties = () => (
    <>
      ...
      <div className={`overflow-hidden transition-all duration-500 ease-in-out ${mobilePropertiesOpen ? 'max-h-96' : 'max-h-0'}`}>
        <ul className="grid grid-cols-2 gap-2 p-2">
          {propertyList.map((property) => (
            <li key={property.href} className="w-full">
              <Link href={property.href} onClick={() => setIsOpen(false)} className=" bg-gray-50 px-2 py-3 hover:bg-gray-100 transition-colors duration-300 ease-in-out border border-gray-200 rounded flex items-center justify-center">
      ...
      {isOpen && (
        <div className="md:hidden absolute top-full left-0 right-0 bg-white shadow rounded-b-md mt-2 z-10 w-full animate-fadeIn">
          <ul className="flex flex-col items-center gap-2 p-4 w-full">
            ...
            {renderNavLink("/mice-group-packages", "Mice/Group packages", () => setIsOpen(false))}
          </ul>
        </div>
      )}
    </nav>
